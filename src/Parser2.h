...
private:
    ...
    std::shared_ptr<ASTNode> parseBaseExpression(size_t scope, std::shared_ptr<ASTNode> origLhs = nullptr);
    std::shared_ptr<ASTNode> parseExpression(size_t scope, std::shared_ptr<ASTNode> origLhs = nullptr);
...
public:
    Parser2(const std::vector<std::filesystem::path> &rtlDirectories, std::filesystem::path path,
