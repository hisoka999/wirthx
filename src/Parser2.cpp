...
Parser2::Parser2(const std::vector<std::filesystem::path> &rtlDirectories, std::filesystem::path path,
...
bool Parser2::hasError() const { return !m_errors.empty(); }
void Parser2::printErrors(std::ostream &outputStream)
{
...
Token Parser2::current() { return m_tokens[m_current]; }
bool Parser2::hasNext() const { return m_current < m_tokens.size(); }
bool Parser2::consume(const TokenType tokenType)
{
...
bool Parser2::tryConsume(const TokenType tokenType)
{
    if (canConsume(tokenType))
    {
        next();
...
bool Parser2::canConsume(TokenType tokenType) const { return canConsume(tokenType, 1); }
bool Parser2::canConsume(TokenType tokenType, size_t next) const
{
    return hasNext() && m_tokens[m_current + next].tokenType == tokenType;
}

bool Parser2::consumeKeyWord(const std::string &keyword)
{
...
bool Parser2::canConsumeKeyWord(const std::string &keyword) const
{
    return canConsume(TokenType::KEYWORD) && iequals(m_tokens[m_current + 1].lexical, keyword);
}

bool Parser2::tryConsumeKeyWord(const std::string &keyword)
{
...
std::optional<std::shared_ptr<VariableType>> Parser2::determinVariableTypeByName(const std::string &name) const
{
...
std::shared_ptr<ASTNode> Parser2::parseEscapedString(const Token &token)
{
...
std::shared_ptr<ASTNode> Parser2::parseNumber()
{
...
bool Parser2::isVariableDefined(const std::string_view name, size_t scope)
{
...
void Parser2::parseTypeDefinitions(const size_t scope)
{
...
std::shared_ptr<ArrayType> Parser2::parseArray(size_t scope)
{
...
std::optional<VariableDefinition> Parser2::parseConstantDefinition(size_t scope)
{
...
std::vector<VariableDefinition> Parser2::parseVariableDefinitions(const size_t scope)
{
...
std::shared_ptr<ASTNode> Parser2::parseLogicalExpression(const size_t scope, std::shared_ptr<ASTNode> lhs)
{
...
std::shared_ptr<ASTNode> Parser2::parseBaseExpression(size_t scope, std::shared_ptr<ASTNode> origLhs)
{
...
std::shared_ptr<ASTNode> Parser2::parseExpression(size_t scope, std::shared_ptr<ASTNode> origLhs)
{
...
std::shared_ptr<ASTNode> Parser2::parseVariableAssignment(size_t scope)
{
    consume(TokenType::NAMEDTOKEN);
...
std::shared_ptr<ASTNode> Parser2::parseVariableAccess(size_t scope)
{
    consume(TokenType::NAMEDTOKEN);
    auto token = current();
    auto variableName = std::string(current().lexical);
...
std::shared_ptr<ASTNode> Parser2::parseToken(size_t scope)
{
...
std::shared_ptr<FunctionDefinitionNode> Parser2::parseFunctionDefinition(size_t scope, bool isFunction)
{
    consume(TokenType::NAMEDTOKEN);
...
std::shared_ptr<ASTNode> Parser2::parseStatement(size_t scope)
{
...
std::shared_ptr<BlockNode> Parser2::parseBlock(size_t scope)
{
...
std::shared_ptr<ASTNode> Parser2::parseKeyword(size_t scope)
{
...
std::shared_ptr<ASTNode> Parser2::parseFunctionCall(size_t scope)
{
    consume(TokenType::NAMEDTOKEN);
...
bool Parser2::importUnit(std::string filename)
{
    ...
    Parser2 parser(m_rtlDirectories, path, tokens);
...
std::unique_ptr<UnitNode> Parser2::parseUnit()
{
    try
    {
